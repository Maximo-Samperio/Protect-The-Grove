...
{
    const float speed = 1.5f;               // Enemy speed (const, flyweight)

    const float maxHealth = 450f;           // Enemy health (const, flyweight)
    private float currentHealth;            // Enemy current health

    public const int value = 150;           // Value in money for turret shop (const, flyweight)
